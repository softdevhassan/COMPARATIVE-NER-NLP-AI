...
def api_compare():
    ...
    if spacy_model:
        try:
            ...
            results["spacy"]["processing_time"] = spacy_results.get("processing_time", 0)
        except Exception as e:
        ...
        try:
            ...
            for i, (word, pred) in enumerate(zip(sentence, predictions)):
                ...
                if pred != "O":
                    ...
                    if pred.startswith("B-") or current_entity is None or current_entity["label"] != label:
                        if current_entity:
